...
function StripePaymentForm({ 
  ...
  return (
    <div className="stripe-payment-form">
      ...
      >
        {processing ? 'Processing Payment...' : `Pay $${finalAmount.toFixed(2)}`}
      </button>
    </div>
  );
}
...
export default function CheckoutPage() {
  ...
  const discountAmount = appliedCoupon ? calculateDiscount(totalAmount, appliedCoupon) : 0;
  ...
  function calculateDiscount(amount, coupon) {
    if (coupon.discountType === 'percentage') {
      ...
      return Math.min(amount, coupon.discountValue); // Fixed amount, can't exceed order total
    }
  }
    ...
    try {
      const response = await fetch("/api/coupons/validate", {
        ...
        body: JSON.stringify({
          ...
          sellerIds: [...new Set(cart.map(item => item.sellerId))]
        })
      });
      ...
      <div className="checkout-container">
        ...
        {orderDetails && (
          <div className="order-details-success">
            ...
            {appliedCoupon && (
              <p><strong>Discount Applied:</strong> -${discountAmount.toFixed(2)} ({appliedCoupon.code})</p>
            )}
      ...
      {cart.length === 0 ? (
        ...
        <form onSubmit={handleSubmit} className="checkout-form">
          ...
          {formData.paymentMethod === "cod" && (
            ...
            >
              {isLoading ? 'Placing Order...' : `Place Order - $${finalAmount.toFixed(2)}`}
            </button>
          )}
