  ...
  return (
    <div className="admin-dashboard">
      ...
      <div className="admin-content">
        {activeTab === 'approvals' && <PendingApprovals authFetch={authFetch} />}
        {activeTab === 'sellers' && <SellersList authFetch={authFetch} />}
        {activeTab === 'buyers' && <BuyersList authFetch={authFetch} />}
      </div>
    </div>
  );
}
...
function PendingApprovals({ authFetch }) {
  ...
  const fetchPendingApprovals = async () => {
    try {
      const response = await authFetch('/api/pending-approvals');
      const data = await response.json();
      
      if (response.ok) {
        setPendingUsers(data);
      } else {
        setMessage(data.message || 'Failed to fetch pending approvals');
      }
    } catch (error) {
  ...
  const handleApprove = async (userId, userName) => {
    try {
      const response = await authFetch(`/api/approve-user/${userId}`, {
        method: 'PATCH',
      });
      
      if (response.ok) {
        setMessage(`✅ ${userName} approved successfully! Approval email sent.`);
  ...
  const handleReject = async (userId, userName) => {
    if (!window.confirm(`Are you sure you want to reject ${userName}'s application? This action cannot be undone.`)) {
      return;
    }

    try {
      const response = await authFetch(`/api/reject-user/${userId}`, {
        method: 'DELETE',
      });
      
      if (response.ok) {
        setMessage(`❌ ${userName} rejected successfully. Notification email sent.`);
...
function SellersList({ authFetch }) {
...
function BuyersList({ authFetch }) {
  ...
  const fetchBuyers = async () => {
    try {
      const response = await authFetch('/api/admin/buyers');
      const data = await response.json();
      if (response.ok) {
        setBuyers(data);
      } else {
        setMessage(data.message || 'Failed to fetch buyers');
      }
    } catch (error) {
